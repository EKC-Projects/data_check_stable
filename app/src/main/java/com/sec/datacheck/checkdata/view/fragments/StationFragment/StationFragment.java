...
public class StationFragment extends Fragment implements View.OnClickListener{
    ...
    @BindView(R.id.station_frag_x_y_coordinates_4_points_sp)
    Spinner X_Y_CoordinatesSpinner;

    @BindView(R.id.station_frag_Grid_Station_sp)
    Spinner gridStationSpinner;

    @BindView(R.id.station_frag_grid_station_name_type_sp)
    Spinner gridStationNameSpinner;

    @BindView(R.id.station_frag_Voltage_Level__132_33__132_13_8_sp)
    Spinner voltageLevelSpinner;


    @BindView(R.id.station_frag_notes_et)
    EditText notes;

    @BindView(R.id.station_frag_save_btn)
    Button saveBtn;
    ...
    public static StationFragment newInstance(MapActivity current, MapPresenter presenter, OnlineQueryResult selectedResult, boolean onlineData) {
        mCurrent = current;
...
//                    }
    ...
    private void init() {
        try {
            ...
            initSpinner(X_Y_CoordinatesSpinner, Columns.STATION.X_Y_Coordinates_4_points);
            initSpinner(gridStationSpinner, Columns.STATION.Grid_Station);
            initSpinner(gridStationNameSpinner, Columns.STATION.Grid_Station_Name);
            initSpinner(voltageLevelSpinner, Columns.STATION.Voltage_Level__132_33__132_13_8);
    ...
    private void saveChanges() {
        try {
            ...
            stationModel.setX_Y_Coordinates_4_points(X_Y_CoordinatesSpinner.getSelectedItemPosition() + 1);
            stationModel.setGrid_Station(gridStationSpinner.getSelectedItemPosition() + 1);
            stationModel.setGrid_Station_Name(gridStationNameSpinner.getSelectedItemPosition() + 1);
